...
registerBlockType("petland/navigation", {
  title: __("Navigation (Petland)", "petlandtextdomain"),
  icon: "admin-post",
  category: "widgets",
  attributes: {
  ...
  edit: (props) => {
    ...
    useEffect(() => {
      apiFetch({
        path: "/wp/v2/block-renderer/petland/navigation",
        method: "POST",
        data: {
          context: "edit",
      ...
      <div {...blockProps}>
        <InspectorControls>
          <PanelBody title={__("Settings", "petlandtextdomain")}>
            <RangeControl
              label={__("Navigation Items to show", "petlandtextdomain")}
              min={1}
              max={50}
              value={postsToShow}
